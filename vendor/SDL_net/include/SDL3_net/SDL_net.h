...
/**
* Enable or disable the Nagle algorithm (TCP_NODELAY) on a stream socket.
*
* By default, TCP sockets use the Nagle algorithm to reduce the number of
* small packets sent over the network. Disabling it (setting TCP_NODELAY)
* can reduce latency for time-sensitive applications at the cost of
* potentially sending more packets.
*
* \param sock the stream socket to configure
* \param enabled SDL_TRUE to disable Nagle (enable TCP_NODELAY),
*                SDL_FALSE to enable Nagle (disable TCP_NODELAY)
* \returns SDL_TRUE on success, SDL_FALSE on failure; call SDL_GetError()
*          for more information.
*
* \since This function is available since SDL_Net 3.X.X.
*
* \sa NET_CreateClient
* \sa NET_AcceptClient
*/
extern SDL_DECLSPEC bool SDLCALL NET_SetStreamSocketNoDelay(NET_StreamSocket *sock, bool enabled);

/**
 * Get the current TCP_NODELAY setting for a stream socket.
 *
 * \param sock the stream socket to query
 * \returns SDL_TRUE if TCP_NODELAY is enabled, SDL_FALSE otherwise.
 *
 * \since This function is available since SDL_Net 3.X.X.
 *
 * \sa NET_SetStreamSocketNoDelay
 */
extern SDL_DECLSPEC bool SDLCALL NET_GetStreamSocketNoDelay(NET_StreamSocket *sock);
