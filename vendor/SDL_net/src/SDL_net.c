...
static bool ApplyTCPNoDelay(Socket handle, bool enabled)
{
#ifdef __EMSCRIPTEN__
    // TCP_NODELAY is not available in Emscripten/WebAssembly
    (void)handle;
    (void)enabled;
    return true;
#else
...
#ifdef SDL_PLATFORM_WINDOWS
    if (setsockopt(handle, IPPROTO_TCP, TCP_NODELAY, (const char *)&flag, sizeof(flag)) == SOCKET_ERROR) {
    ...
    if (setsockopt(handle, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag)) < 0) {
        return SetSocketErrorBool("Failed to set TCP_NODELAY", errno);
    }
#endif
...
bool NET_SetStreamSocketNoDelay(NET_StreamSocket *sock, bool enabled)
{
    if (!sock) {
        return SDL_InvalidParamError("sock");
    }
    ...
    if (sock->handle == INVALID_SOCKET) {
        return SDL_SetError("Socket handle is invalid");
    }

    if (!ApplyTCPNoDelay(sock->handle, enabled)) {
        return false;
    }

    sock->noDelay = enabled;
    return true;
}

bool NET_GetStreamSocketNoDelay(NET_StreamSocket *sock)
{
    if (!sock) {
